/**
 * Generated by orval v8.6.2 🍺
 * Do not edit manually.
 * Aerisun API
 * OpenAPI spec version: 0.1.0
 */
import type { AerisunDomainSiteConfigSchemasCommunitySurfaceRead } from './aerisunDomainSiteConfigSchemasCommunitySurfaceRead';
import type { CommunityConfigReadAvatarPresetsItem } from './communityConfigReadAvatarPresetsItem';

export interface CommunityConfigRead {
  /** Comment provider name */
  provider: string;
  /** Comment server URL */
  server_url: string;
  /** Comment-enabled surfaces */
  surfaces: AerisunDomainSiteConfigSchemasCommunitySurfaceRead[];
  /** Commenter metadata fields */
  meta: string[];
  /** Required metadata fields */
  required_meta: string[];
  /** Emoji preset CDN URLs */
  emoji_presets: string[];
  /** Emoji search enabled */
  enable_enjoy_search: boolean;
  /** Image uploads allowed */
  image_uploader: boolean;
  /** Authentication mode */
  login_mode: string;
  /** OAuth endpoint URL */
  oauth_url: string | null;
  /** OAuth provider names */
  oauth_providers: string[];
  /** Anonymous commenting allowed */
  anonymous_enabled: boolean;
  /** Moderation mode */
  moderation_mode: string;
  /** Default sort order */
  default_sorting: string;
  /** Comments per page */
  page_size: number;
  /** Max image upload size in bytes */
  image_max_bytes?: number | null;
  /** Predefined avatar options */
  avatar_presets: CommunityConfigReadAvatarPresetsItem[];
  /** Guest avatar mode */
  guest_avatar_mode: string;
  /** Draft saving enabled */
  draft_enabled: boolean;
  /** Avatar resolution strategy */
  avatar_strategy: string;
  /** Avatar helper text */
  avatar_helper_copy: string;
  /** Migration progress state */
  migration_state: string;
}
