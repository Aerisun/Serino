...
router = APIRouter(prefix="/object-storage", tags=["admin-object-storage"])


@router.get("/config", response_model=ObjectStorageConfigRead, summary="获取 OSS 加速配置")
...
def put_object_storage_config(
    ...
    update_object_storage_config(session, payload)
    result = refresh_object_storage_health_status(session)
...
@router.post("/config/test", response_model=ObjectStorageHealthRead, summary="测试 OSS 加速配置")
...
@router.get("/sync-records", summary="获取 OSS 同步记录")
def get_object_storage_sync_records(
    page: int = 1,
    page_size: int = 20,
    q: str | None = None,
    _admin: AdminUser = Depends(get_current_admin),
