...
export function ProfileTab() {
  ...
  useEffect(() => {
    if (profile) {
      setForm({ name: profile.name, title: profile.title, bio: profile.bio, role: profile.role, footer_text: profile.footer_text, hero_video_url: profile.hero_video_url || "" });
  ...
  return (
    <>
      ...
      <div className="rounded-lg border bg-card p-6 mt-6 max-w-2xl">
        <h3 className="text-lg font-semibold mb-1">{t("siteConfig.featureFlags")}</h3>
        <p className="text-sm text-muted-foreground mb-4">{t("siteConfig.featureFlagsDescription")}</p>
        <div className="space-y-4">
          {[
            { key: "toc", label: t("siteConfig.featureToc"), desc: t("siteConfig.featureTocDesc") },
            { key: "reading_progress", label: t("siteConfig.featureReadingProgress"), desc: t("siteConfig.featureReadingProgressDesc") },
            { key: "social_sharing", label: t("siteConfig.featureSocialSharing"), desc: t("siteConfig.featureSocialSharingDesc") },
          ].map((flag) => (
            <label key={flag.key} className="flex items-center justify-between gap-4 py-2">
              <div>
              ...
              <button
                ...
                onClick={() => setFeatureFlags((prev) => ({ ...prev, [flag.key]: !(prev[flag.key] ?? true) }))}
                className={cn(
              ...
              >
                <span
                  className={cn(
                    ...
                    (featureFlags[flag.key] ?? true) ? "translate-x-4" : "translate-x-0"
                  )}
                />
              </button>
            </label>
