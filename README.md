...
- 🔌 **MCP 原生支持**：内建标准 MCP API 与 skills，借助权限域安全对接 openclaw/codex/claude。
...
项目主体功能已经可以在生产环境下稳定运行，极少部分高阶能力仍在持续完善。系统会自动发现并提醒更新，后续可在管理台一键更新系统。
...
- [x] 添加集成型的高级配置(API，OSS，SMTP，代理，备份连接，MCP服务的有效性)日常诊断和失效提醒功能 `8月9日完成`
- [x] 将 MCP 升级为以 2026-07-28 为首选并自动兼容当前客户端，同时提供 Codex / Claude Code 共用 Skills 与域名安装入口 `8月9日完成`
