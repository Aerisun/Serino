...
export default function ExcerptListPage() {
  ...
  const { data: listData, isLoading } = useListExcerpts(params);
  ...
  const { mutateAsync: bulkDelete, isPending: isBulkDeleting } = useBulkDeleteExcerpts({
    mutation: {
      onSuccess: () => {
  ...
  const { mutateAsync: bulkStatus } = useBulkStatusExcerpts({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListExcerptsQueryKey() });
      },
