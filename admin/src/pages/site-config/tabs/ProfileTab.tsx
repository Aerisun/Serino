...
const PROFILE_FIELD_COPY: Record<"zh" | "en", Record<ProfileCopyFieldKey, FieldHelpCopy>> = {
  zh: {
    ...
    og_image: {
      label: "首页静态兜底图",
      title: "首页背景媒体不可用时显示的静态图片",
      description: "用于首页背景视频不可用时的静态背景，并在 Hero 翻转视觉图为空时参与兜底。建议使用适合铺满首屏的横向图片。",
      usageTitle: "会影响这些位置",
      usageItems: [
        "首页背景视频缺失或报错时的背景图",
        "Hero 翻转视觉图为空时的图片兜底",
      ],
      placeholder: "上传或填写首页静态兜底图地址",
      note: "首页背景与 Hero 视觉兜底图",
    },
    site_icon_url: {
      label: "浏览器标签图标",
      title: "浏览器标签页左侧的小图标",
  ...
  en: {
    ...
    og_image: {
      label: "Homepage Static Fallback",
      title: "The static image shown when homepage media is unavailable",
      description: "Used as the static homepage background when the video is unavailable, and as the fallback when the hero flip image is empty. A landscape image that can fill the first screen works best.",
      usageTitle: "Used in",
      usageItems: [
        "Homepage background when the hero video is missing or fails",
        "Hero image fallback when the flip image is empty",
      ],
      placeholder: "Upload or paste the homepage static fallback image URL",
      note: "Homepage background and hero fallback image",
    },
    site_icon_url: {
