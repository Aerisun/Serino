...
_DISPATCHER_DIR_NAME = "active"
_DISPATCHER_FILE_NAME = "routes.caddy"
...
def _validate_slug(slug: str, settings: Settings | None = None) -> None:
    ...
    if slug.split("/", 1)[0] in reserved_slugs:
...
def _dispatcher_path(settings: Settings) -> Path:
    return settings.caddy_routes_dir.expanduser().resolve() / _DISPATCHER_DIR_NAME / _DISPATCHER_FILE_NAME


def _render_route_dispatcher(routes_dir: Path) -> str:
    route_files = list(routes_dir.glob("route-*.caddy"))

    def sort_key(route_file: Path) -> tuple[int, str, str]:
        route_path = _read_metadata(route_file).get("serino-route-path", "")
        return (-len(route_path), route_path, route_file.name)

    sections = [
        "# Generated by Serino. Source rules remain in route-*.caddy files.",
        "",
    ]
    for route_file in sorted(route_files, key=sort_key):
        sections.append(route_file.read_text(encoding="utf-8").rstrip("\n"))
        sections.append("")
    return "\n".join(sections)


def _write_route_dispatcher(settings: Settings) -> None:
    routes_dir = settings.caddy_routes_dir.expanduser().resolve()
    _atomic_write(_dispatcher_path(settings), _render_route_dispatcher(routes_dir))


def ensure_route_dispatcher(settings: Settings | None = None) -> None:
    """Rebuild the ordered route dispatcher before Caddy starts."""
    active_settings = settings or get_settings()
    routes_dir = active_settings.caddy_routes_dir.expanduser().resolve()
    with _routes_lock(routes_dir):
        _write_route_dispatcher(active_settings)
...
def _reload_or_rollback(settings: Settings, snapshots: dict[Path, bytes | None]) -> None:
    dispatcher_path = _dispatcher_path(settings)
    if dispatcher_path not in snapshots:
        snapshots[dispatcher_path] = dispatcher_path.read_bytes() if dispatcher_path.exists() else None
    try:
        _write_route_dispatcher(settings)
