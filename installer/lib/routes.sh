...
caddy_route_dispatcher_file() {
  printf '%s/active/routes.caddy' "${SERINO_CADDY_ROUTES_DIR}"
}
...
rebuild_caddy_route_dispatcher() {
  ...
  if ! run_as_root install -d -o root -g "${SERINO_SERVICE_GROUP}" -m 0770 "$(dirname "${dispatcher_file}")"; then
    rm -f "${temp_file}"
    return 1
  fi
  if ! run_as_root python3 - "${SERINO_CADDY_ROUTES_DIR}" "${temp_file}" <<'PY'
...
for route_file in routes_dir.glob("route-*.caddy"):
    content = route_file.read_text(encoding="utf-8").rstrip("\n")
    route_path = ""
    for line in content.splitlines():
        if line.startswith("# serino-route-path: "):
            route_path = line.removeprefix("# serino-route-path: ")
            break
    entries.append((-len(route_path), route_path, route_file.name, content))
...
for _path_length, _route_path, _filename, content in entries:
...
PY
  ...
  if ! run_as_root install -o root -g "${SERINO_SERVICE_GROUP}" -m 0640 "${temp_file}" "${dispatcher_file}"; then
    rm -f "${temp_file}"
    return 1
  fi
  rm -f "${temp_file}"
