...
function InlineToggle({ checked, onCheckedChange, ariaLabel }: InlineToggleProps) {
  return (
    <button
      type="button"
      role="switch"
    ...
    <div className={cn("space-y-2", className)}>
      <div className="flex min-w-0 flex-wrap items-center gap-x-8 gap-y-2">
        {rssExclusion ? (
          ...
          >
            <LabelWithHelp
              label={rssExclusion.label}
              title={rssExclusion.label}
              description={rssExclusion.helpDescription}
              className="gap-1.5 whitespace-nowrap [&>label]:tracking-tight [&>label]:text-foreground/92"
            />
            <InlineToggle
              checked={rssExclusion.checked}
              onCheckedChange={rssExclusion.onCheckedChange}
              ariaLabel={rssExclusion.ariaLabel}
            />
          </div>
        ) : null}
        ...
        >
          ...
          <InlineToggle
            ...
            ariaLabel="Toggle custom publish time"
          />
        </div>
        ...
        <div
          className={cn(
            "order-3 min-w-0 overflow-hidden transition-[max-width,opacity,margin] duration-200 ease-in-out",
            isCustom
              ? "ml-3 max-w-[18rem] flex-1 opacity-100 sm:ml-5 max-md:ml-0"
