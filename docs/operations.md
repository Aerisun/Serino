...
1. 填写备份机 IPv4 地址或域名。
2. 点击“检测”。如果 `serino-backup` 已经能连接，会直接检查远端备份历史。
3. 如果检测失败，生成临时命令，在备份机上用 sudo 执行。
4. 新建或覆盖备份历史时设置恢复密码。
5. 回到后台点击“开始备份”。

临时命令默认 10 分钟过期、单次使用。重新生成同一备份目标的命令时，旧的 pending 命令会自动撤销。

临时脚本只包含 Serino 的 SSH 公钥、目标用户名、目标目录、回传地址和过期信息；不会包含管理员 token、SSH 私钥、恢复密码、恢复钥匙包或数据库内容。脚本会在备份机上创建或复用 `serino-backup` 专用用户、创建 `/srv/serino-backups` 目录，并用 Serino 标记区块维护 `authorized_keys`。

远端备份仓库会写入 `repo.json` 身份文件。检测到同一仓库时可以继续增量备份；检测到另一套历史时，后台只提供“从备份机历史恢复数据”和“覆盖远端历史”，不会自动混写。
