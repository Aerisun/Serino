...
export default function ThoughtListPage() {
  ...
  const { data: listData, isLoading } = useListThoughts(params);
  ...
  const { mutateAsync: bulkDelete, isPending: isBulkDeleting } = useBulkDeleteThoughts({
    mutation: {
      onSuccess: () => {
  ...
  const { mutateAsync: bulkStatus } = useBulkStatusThoughts({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListThoughtsQueryKey() });
      },
